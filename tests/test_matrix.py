...
@pytest.mark.parametrize("path", ["src/shared.py", "tests/test_shared.py", "builder/lock.toml", ".github/workflows/build.yml", "pyproject.toml", "uv.lock", "packages/shared.py", "src/readme.py"])
...
def test_package_rename_sees_new_owner(tmp_path):
    ...
    package_toml.write_text(package_toml.read_text().replace('name = "alpha"', 'name = "gamma"').replace("alpha =", "gamma ="))
